...
public class ManageProductTest extends Base {
	...
	public void verifyIfUserIsAbleToCreateNewProduct() throws IOException, AWTException {
		...
		manageproductpage.clickOnNewIcon().enterValueOnTitleField(titlevalue).enterValueOnWeightvalueField(wightvalue)
				.selectWeightunitDropdownValue().enterValueOnMaxQuantityField(maxqty).enterPrice(pricevalue)
				.enterStock(stockvalue).fileUpload().selectFeaturedRadiobutton().clickOnSavebutton();
		boolean alertdisplayed = manageproductpage.isAlertMessageDisplayed();
		Assert.assertTrue(alertdisplayed, Constants.MANAGEPRODUCT_MESSAGE);
	}
